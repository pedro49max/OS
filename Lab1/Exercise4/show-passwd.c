	...
	for (i=0; i<nr_entries; i++) {
		...
		case PIPE_MODE:
			fprintf(options->outfile,"|%s|%s|%d|%d|%s|%s|%s|\n",
			        ...
			        e->uid, e->gid,e->user_name,
			        e->user_home, e->user_shell);
		case CSV_MODE:  // New CSV mode using commas instead of colons
			fprintf(options->outfile, "%s,%s,%d,%d,%s,%s,%s\n",
			        e->login_name, e->optional_encrypted_passwd,
			        e->uid, e->gid, e->user_name,
			        e->user_home, e->user_shell);
...
int main(int argc, char *argv[])
{
	...
	options.inputfile = "/etc/passwd"; //new default

	/* Parse command-line options */
	while((opt = getopt(argc, argv, "hvpcio:")) != -1) {
		switch(opt) {
		case 'h':
			fprintf(stderr,"Usage: %s [ -h | -v | -p | -c | -i <input_file> | -o <output_file> ]\n",argv[0]);
			exit(0);
		case 'v':
			options.output_mode=VERBOSE_MODE;
			break;
		case 'p':
			...
			break;
		case 'c':
			options.output_mode = CSV_MODE;  /* New CSV output mode */
			break;
		case 'i':
			options.inputfile = argv[0];  /* Input file specified by the user */
			break;
		case 'o':
			if ((options.outfile=fopen(argv[0],"wx"))==NULL) {
...
/*
1. In clone string you allocate some memory and copy the contents of the original string, with the other two methods you will have two
 pointers to the same memory point and you could modify the content of one pointer while using the other one

2. The ':' are transform into null values to saperate the string in pieces, and the line pointer points to the next piece of the string
*/